...
class window.QuizCoordinator
  ...
  constructor: (@elem, @emotionVideoStore, @fbInteractor, @currentPowerup) ->
    @quizProbability = 1
...
class window.ChatRoom
  ...
  constructor: (@fbInteractor, @videoRecorder) ->
    ...
    @currentPowerup = new Powerup($('#powerup_container'))
    @currentPowerup.render()
    @quizCoordinator = new QuizCoordinator($("#quiz_container"), @emotionVideoStore, @fbInteractor, @currentPowerup)
